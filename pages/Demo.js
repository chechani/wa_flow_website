            ...
            <div style={{ display: 'flex', gap: '10px' }}>
                ...
                return (
                  <div style={{ width: "350px", border: '1px solid #ddd', borderRadius: '5px', padding: '20px', backgroundColor: '#fff' }}>
                    ...
                    <div id="content-input-container">
                      {['large-heading', 'small-heading', 'caption', 'body'].includes(contentType) && (
                        <input
                          type="text"
                          id="content-text"
              ...
              const ContentPreview = ({
                ...
                const editContent = (index) => {
                  ...
                  setContentLabel(content.label || '');
                  setContentText(content.content || '');
                  setContentMedia(null);
                  setContentList(
                    content.type === 'checkbox' || content.type === 'radio' || content.type === 'dropdown'
                      ? content.content.map(option => option.title).join('\n')
                      : ''
                  );
                };
                  ...
                  <div style={{ width: '600px', height: 'auto', overflowY: 'auto', border: '1px solid #ddd', borderRadius: '5px' }}>
                    {currentScreenIndex === null ? (
                      ...
                      screens[currentScreenIndex].content.map((item, index) => (
                        <div key={index} style={{ margin: '8px', cursor: 'move', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: '5px', border: '1px solid #ddd', borderRadius: '5px', backgroundColor: '#fff' }} className="content-item">
                          <div style={{ flexGrow: 1 }}>
                            ...
                            {item.type === 'text-input' && <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }} className="label-container"><label style={{ minWidth: '100px' }}>{item.label}</label><input type="text" style={{ height: '30px' }} placeholder={item.label} /></div>}
                            {item.type === 'text-area' && <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }} className="label-container"><label style={{ minWidth: '100px' }}>{item.label}</label><textarea style={{ height: '60px' }} placeholder={item.content} /></div>}
                            {item.type === 'media' && <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }} className="label-container"><img src={item.content} alt={item.label} style={{ maxWidth: '100px', maxHeight: '100px' }} className="preview-image" /><input type="file" /></div>}
                            {item.type === 'checkbox' && <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }} className="label-container"><label style={{ minWidth: '100px' }}>{item.label}</label><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '10px' }} className="option-group">{item.content.map(option => <div key={option.id}><input type="checkbox" name={item.label} /><label>{option.title}</label></div>)}</div></div>}
                            {item.type === 'radio' && <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }} className="label-container"><label style={{ minWidth: '100px' }}>{item.label}</label><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '10px' }} className="option-group">{item.content.map(option => <div key={option.id}><input type="radio" name={item.label} /><label>{option.title}</label></div>)}</div></div>}
                            {item.type === 'dropdown' && <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }} className="label-container"><label style={{ minWidth: '100px' }}>{item.label}</label><select style={{ minWidth: '200px' }} className="fixed-length-dropdown">{item.content.map(option => <option key={option.id}>{option.title}</option>)}</select></div>}
                          </div>
                          <div style={{ display: 'flex', gap: '10px' }} className="button-group">
                            <span style={{ cursor: 'pointer', fontWeight: 'bold', color: 'red', marginLeft: "5px", marginTop: "-15px" }} className="delete-btn" onClick={() => deleteContent(index)}>x</span>
                            <span style={{ cursor: 'pointer', fontWeight: 'bold', color: 'green', marginLeft: "5px", marginTop: "-15px" }} className="edit-btn" onClick={() => editContent(index)}>✎</span>
                          </div>
                        </div>
                      ))
                    )}
